...
app.post("/make-server-e56751be/init-data", async (c) => {
  try {
...
async function logConsumption(userId: string, calories: number, items: string[], orderDate?: string) {
  // Use order date if provided, otherwise use today
  ...
  // Also maintain a list of all consumption dates for the user
...
// New endpoint: Get all consumption history
app.get("/make-server-e56751be/consumption/all", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
app.get("/make-server-e56751be/products", async (c) => {
  try {
...
app.get("/make-server-e56751be/articles", async (c) => {
  try {
...
// Get all products
app.get("/make-server-e56751be/admin/products", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Create product
app.post("/make-server-e56751be/admin/products", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Update product
app.put("/make-server-e56751be/admin/products/:id", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Delete product
app.delete("/make-server-e56751be/admin/products/:id", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Get all articles
app.get("/make-server-e56751be/admin/articles", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Create article
app.post("/make-server-e56751be/admin/articles", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Update article
app.put("/make-server-e56751be/admin/articles/:id", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Delete article
app.delete("/make-server-e56751be/admin/articles/:id", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
app.get("/make-server-e56751be/admin/orders", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Update order status
app.put("/make-server-e56751be/admin/orders/:id/status", async (c) => {
  const user = await verifyUser(c.req.header('Authorization'));
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
